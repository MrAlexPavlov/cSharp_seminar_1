...
{
    int result = 0;
    int a = Math.Abs(digitNum);//на тот случай если пользователь ввел отрицательное число

    while (a >= 1)//Развораыиваем число
    {
        result = result * 10 + (a % 10);
        a = a / 10;
    }

    if (digitNum < 0) result = -result;//Если первоначальное число было отрицательным возвращаем так же отрицательное
    return result;
}
...
if (number == altNumber) // Если первоночальное число равно зеркальному - это число палиндром
{
